...
void _StgFEM_StandardConditionFunctions_AssignFromXML( void* component, Stg_ComponentFactory* cf, void* data ) {
	...
	condFunc = ConditionFunction_New( StgFEM_StandardConditionFunctions_GaussianDistribution1, (Name)"GaussianDistribution1" );
	ConditionFunction_Register_Add( condFunc_Register, condFunc );
...
void StG_FEM_StandardConditionFunctions_StepFunctionProduct2( Node_LocalIndex node_lI, Variable_Index var_I, void* _context, void* _result ) {
	...
	defaultValue = Dictionary_GetDouble_WithDefault( dictionary, "StepFunctionProduct2DefaultValue", 0.0 );
...
void StgFEM_StandardConditionFunctions_GaussianDistribution( Node_LocalIndex node_lI, Variable_Index var_I, void* _context, void* _result ) {
	...
	expTerm =  exp( -distsq / ( 2.0 * sigma * sigma )  );
	*result = gaussianScale * pow(expTerm, 2.0) + background;
#endif
	for( dim_I = 0; dim_I < nDims-1; dim_I++ )
		distsq += ( coord[dim_I] - orig[dim_I] ) * ( coord[dim_I] - orig[dim_I] );
	dist = sqrt(distsq)/sigma*M_PI;
	if(dist>M_PI) *result = background;
	else {
		cosPower = pow(cos(dist),3.0)+1;
		cosFactor = (cosPower>1.0?1.0:cosPower);
		*result = gaussianScale*cosFactor+background;
	}
	//*result = (dist>sigma?background:(gaussianScale+background));
}

void StgFEM_StandardConditionFunctions_GaussianDistribution1( Node_LocalIndex node_lI, Variable_Index var_I, void* _context, void* _result ) {
	FiniteElementContext *	context            = (FiniteElementContext*)_context;
	FeVariable*             feVariable         = NULL;
	Dictionary*             dictionary         = context->dictionary;
	double*                 result             = (double*) _result;
	...
	unsigned		dim_I;
	double			orig[3];
	double			sigma              = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"sigma1", 1.0  );
	double			gaussianScale      = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"GaussianScale1", 1.0  );
	double			background         = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"backgroundValue1", 1.0  );
	double			distsq             = 0.0;
	double			dist               = 0.0;
	double          expTerm            = 0.0;
	double          cosPower           = 0.0;
	...
	orig[0] = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"x01", 0.0  );
	orig[1] = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"y01", 0.0  );
	orig[2] = Dictionary_GetDouble_WithDefault( dictionary, (Dictionary_Entry_Key)"z01", 0.0 );
